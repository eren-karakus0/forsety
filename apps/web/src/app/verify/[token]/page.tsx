  ...
  if (status === "loading") {
    return (
      <div className="flex flex-col items-center gap-4 py-20">
        ...
        <p className="text-sm text-navy-300">Verifying evidence pack...</p>
      </div>
    );
  }
      ...
      <Card className={`overflow-hidden rounded-xl ${banner.borderColor} ${banner.bgColor}`}>
        <CardContent className="flex items-center gap-4 p-6">
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-3 text-sm text-navy-300">
            ...
            <Card className="rounded-xl border-white/10 bg-white/5">
              <CardHeader className="border-b border-white/10">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-sm font-semibold text-white">Dataset</CardTitle>
                </div>
              </CardHeader>
              <CardContent className="pt-4 text-sm">
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="text-white">{pack.dataset.name}</span>
                  </div>
                  {pack.dataset.blobHash && (
                    <div className="flex justify-between">
                      <span className="text-navy-400 text-xs uppercase tracking-wider font-semibold">Blob Hash</span>
                      <span className="font-mono text-xs text-navy-300 break-all max-w-[60%] text-right">{pack.dataset.blobHash}</span>
                    </div>
                  )}
                  {pack.dataset.ownerAddress && (
                    <div className="flex justify-between">
                      <span className="text-navy-400 text-xs uppercase tracking-wider font-semibold">Owner</span>
                      <span className="font-mono text-xs text-navy-300">{pack.dataset.ownerAddress}</span>
                    </div>
                  )}
                </div>
              </CardContent>
            ...
            <Card className="rounded-xl border-white/10 bg-white/5">
              <CardHeader className="border-b border-white/10">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-sm font-semibold text-white">Licenses</CardTitle>
                </div>
              </CardHeader>
            ...
            <Card className="rounded-xl border-white/10 bg-white/5">
              <CardHeader className="border-b border-white/10">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-sm font-semibold text-white">Policies ({pack.policies.length})</CardTitle>
                </div>
              </CardHeader>
            ...
            <Card className="rounded-xl border-white/10 bg-white/5">
              <CardHeader className="border-b border-white/10">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-sm font-semibold text-white">Access Log ({pack.accessLog.length})</CardTitle>
                </div>
              </CardHeader>
              <CardContent className="p-0">
                <div className="divide-y divide-white/10">
                  {pack.accessLog.map((log, i) => (
                    <div key={i} className="flex items-center justify-between px-5 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-mono text-xs text-navy-300">{log.accessorAddress}</span>
                      </div>
                      {log.timestamp && (
            ...
            <CardContent className="pt-4 text-sm">
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="mt-1 font-mono text-[10px] text-navy-300 break-all">
                    {data.pack.packHash}
                  </p>
                </div>
              </div>
